...
### Generate test data for basis function testing
# Test data creation helper
def create_test_data(num_paths=10_000, num_steps=6):
    """Create test data for basis function testing
    
    >>> bs = create_test_data(num_paths=3, num_steps=2)
    >>> isinstance(bs, DiscreteBlackScholes)
    True
    >>> bs.sVals.shape == (3, 3)  # (num_paths, num_steps + 1)
    True
    """
    ...
    for step in range(numSteps):
        sVals[:, step+1] = sVals[:, step] * np.exp( (mu - 1/2 * vol ** 2)*dt + (vol * np.sqrt(dt) * Z1[:, step+1]))
...
if __name__ == '__main__':
    ...
    # Re-import sklearn here to avoid module-level import issues
    from sklearn.preprocessing import PolynomialFeatures
    

    
    def test_bspline_basis(num_basis=4):
        """Test BSplineBasis implementation
        
        >>> bs = create_test_data(num_paths=2, num_steps=1)
        >>> basis = BSplineBasis()
        >>> data = basis.compute_basis(bs.sVals, num_basis=4)
        >>> data.shape == (2, 2, 4)  # (steps+1, paths, basis)
        True
        """
        bs = create_test_data(num_paths=2, num_steps=1)
        basis = BSplineBasis()
        data = basis.compute_basis(bs.sVals)
        print("BSpline test passed!")
        return True
        """Test BSplineBasis implementation
        
        >>> bs = create_test_data(2, 1)
        >>> basis = BSplineBasis(p=3)
        >>> data = basis.compute_basis(X, num_basis=4)
        >>> data.shape == (2, 2, 4)  # (steps+1, paths, basis)
        True
        """
        X = create_test_data()
    ...
    def test_polynomial_basis():
        """Test PolynomialBasis implementation
        
        >>> bs = create_test_data(2, 1)
        >>> basis = PolynomialBasis()
        >>> x = np.array([[2.0], [3.0]])
        >>> data = basis.compute_basis(x, num_basis=3)
        >>> np.allclose(data[0, :, 0], 1.0)  # bias term
        True
        >>> np.allclose(data[0, :, 1], [2.0, 3.0])  # linear term
        True
        >>> np.allclose(data[0, :, 2], [4.0, 9.0])  # quadratic term
        True
        """
        X = create_test_data()
        basis = PolynomialBasis()
    ...
    def test_sklearn_basis():
        """Test SKLearnPolynomialBasis implementation
    ...
    def test_spline_transformer():
        """Test SKLearnSplineTransformer implementation
        
        >>> X = create_test_data(num_paths=2, num_steps=1)
        >>> basis = SKLearnSplineTransformer(degree=3, n_knots=4)
        >>> data = basis.compute_basis(X, num_basis=6)  # 6 basis functions
        >>> data.shape == (2, 2, 6)  # (steps+1, paths, basis)
        True
        >>> np.all(np.isfinite(data))  # Check all values are finite
        True
        """
        X = create_test_data()
        basis = SKLearnSplineTransformer(p=3, ncolloc=4)
        data = basis.compute_basis(X6)
        print("Spline transformer test passed!")
        return True
