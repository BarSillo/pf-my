...
\section*{Mathematical Formulation of Realized Delta}
...
1. \textbf{Black-Scholes Delta at Stock Price} \( S' \):
...
Here:
\begin{itemize}
    \item \( K = S \cdot e^{-\text{log-moneyness}} \) is the rescaled strike price.
...
2. \textbf{Log-Normal Probability Density Function of Stock Price} \( S' \):
...
3. \textbf{Bounds of Integration}:
\[
S_{\text{low}} = S \cdot e^{-3\sigma}, \quad S_{\text{high}} = S \cdot e^{3\sigma}
\]

4. \textbf{Normalization by \( 3\sigma \)}:
